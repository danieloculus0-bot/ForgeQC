...
def parse_bom_candidates(text):
    rows = []
    material_markers = ["steel", "alum", "aluminum", "stainless", "brass", "copper", "plastic", "poly", "rubber", "galv", "crs", "hrs", "sheet", "plate"]
    ...
    for raw in text.splitlines():
        ...
        confidence = 0.35
        confidence += 0.15 if item_no else 0
        confidence += 0.25 if part_number else 0
        confidence += 0.15 if quantity else 0
        confidence += 0.10 if description_words else 0
        confidence += 0.10 if material_words else 0
...
def create_bom_from_pdf(pdf_path, source_filename, source="Quoting Drawing"):
    ...
    upload = BOMUpload(
        ...
        extraction_status="Review Needed" if rows else "No BOM Found",
        extraction_notes=error or f"Extracted {len(rows)} candidate BOM line(s).",
    )
    db.session.add(upload)
    db.session.flush()

    for row in rows:
        db.session.add(BOMLine(upload_id=upload.id, **row))
...
def create_app():
    ...
    def quoting_resources():
        if request.method == "POST":
            ...
            if not drawing or not drawing.filename.lower().endswith(".pdf"):
                log("Quoting", "Upload failed", "Customer drawing PDF missing")
                db.session.commit()
            ...
            quote = QuoteResource(
                ...
                drawing_filename=safe_name,
                drawing_path=str(stored),
                notes=request.form.get("notes"),
            )
            db.session.add(quote)
            db.session.flush()

            bom_upload, rows = create_bom_from_pdf(stored, safe_name, source="Quoting Resource")
            quote.bom_upload_id = bom_upload.id
            quote.status = "BOM Review Needed" if rows else "Drawing Saved - No BOM Found"

            log("Quoting", "Customer drawing entered", f"{quote.quote_number}; auto BOM candidates: {len(rows)}")
            db.session.commit()
            return redirect(f"/quoting/{quote.id}")

        records = "".join(
            f"<tr><td><a href='/quoting/{q.id}'>{q.quote_number}</a></td><td>{q.customer.name if q.customer else ''}</td><td>{q.part_number or ''}</td><td>{q.drawing_filename or ''}</td><td>{q.status}</td></tr>"
            for q in QuoteResource.query.order_by(QuoteResource.id.desc()).limit(100)
        )
        body = f"""
        <div class='notice'>Entering a customer drawing here automatically runs BOM extraction and creates a reviewable BOM record. Drawings stay local runtime data.</div>
        <section><form method='post' enctype='multipart/form-data' class='form'>
        <label>Quote Number<input name='quote_number' required></label>
        <label>Customer<input name='customer'></label>
        <label>Part Number<input name='part_number'></label>
        <label class='wide'>Part Description<input name='part_description'></label>
        <label class='wide'>Customer Drawing PDF<input type='file' name='drawing_pdf' accept='application/pdf' required></label>
        <label class='wide'>Notes<textarea name='notes'></textarea></label>
        <button>Save Drawing And Auto Extract BOM</button>
        </form></section><br>
        <section><h2>Quoting Resources</h2><table><tr><th>Quote</th><th>Customer</th><th>Part</th><th>Drawing</th><th>Status</th></tr>{records}</table></section>
        """
    ...
    def quote_detail(quote_id):
        ...
        link = f"<a class='btn' href='/bom/{quote.bom_upload_id}'>Review Extracted BOM</a>" if quote.bom_upload_id else "<span class='muted'>No BOM record</span>"
        body = f"""
        <section><h2>{quote.quote_number}</h2>
        <table>
        <tr><th>Customer</th><td>{quote.customer.name if quote.customer else ''}</td></tr>
        <tr><th>Part</th><td>{quote.part_number or ''}</td></tr>
        <tr><th>Drawing</th><td>{quote.drawing_filename or ''}</td></tr>
        <tr><th>Status</th><td>{quote.status}</td></tr>
        <tr><th>BOM</th><td>{link}</td></tr>
        </table></section>
        """
    ...
    def bom_review(upload_id):
        ...
        if request.method == "POST":
            for line in lines:
                ...
                line.item_no = request.form.get(prefix + "item_no")
                line.part_number = request.form.get(prefix + "part_number")
                line.description = request.form.get(prefix + "description")
                line.material = request.form.get(prefix + "material")
                line.revision = request.form.get(prefix + "revision")
                try:
                    line.quantity = float(request.form.get(prefix + "quantity") or 0)
                except ValueError:
                    line.quantity = 0
                line.approved = request.form.get(prefix + "approved") == "on"
            upload.extraction_status = "Reviewed"
            log("BOM", "Review saved", upload.source_filename)
            db.session.commit()
            return redirect(f"/bom/{upload.id}")
        ...
        for line in lines:
            ...
            rows += f"""
            <tr>
            <td><input type='checkbox' name='line_{line.id}_approved' {checked}></td>
            <td><input name='line_{line.id}_item_no' value='{line.item_no or ''}'></td>
            <td><input name='line_{line.id}_part_number' value='{line.part_number or ''}'></td>
            <td><input name='line_{line.id}_description' value='{line.description or ''}'></td>
            <td><input name='line_{line.id}_material' value='{line.material or ''}'></td>
            <td><input name='line_{line.id}_quantity' value='{line.quantity or 0}'></td>
            <td><input name='line_{line.id}_revision' value='{line.revision or ''}'></td>
            <td>{line.confidence}</td>
            <td>{line.raw_text or ''}</td>
            </tr>
            """
        body = f"""
        <div class='notice'>Review before using for quoting, purchasing, or routing. Extraction is automatic, approval is human-controlled.</div>
        <section><h2>{upload.source_filename}</h2><p class='muted'>{upload.extraction_notes or ''}</p>
        <form method='post'><table><tr><th>Use</th><th>Item</th><th>Part Number</th><th>Description</th><th>Material</th><th>Qty</th><th>Rev</th><th>Confidence</th><th>Raw Text</th></tr>{rows}</table><br><button>Save BOM Review</button></form></section>
        """
